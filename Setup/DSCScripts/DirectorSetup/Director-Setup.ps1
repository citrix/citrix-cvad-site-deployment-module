Configuration DirectorSetup
{
    ...
    $logFilePath = "$($CitrixModulesPath)\Director_Installation.log"
    $xenDesktopServerSetupPath = "$($CitrixModulesPath)\x64\XenDesktop Setup\XenDesktopServerSetup.exe"
    $directorConfigPath = "C:\inetpub\wwwroot\Director\tools\DirectorConfig.exe"
    ...
    Node localhost {
        ...
        Script DomainJoin {
            DependsOn  = @("[File]CitrixModules", "[WindowsFeature]IISInstall", "[Registry]EnsureCvadDownloadRegistryPresents", "[Script]EnableNetworkDiscovery")
        ...
        #region Director Installation
        Script InstallDirector {
            ...
            SetScript            = {
                Add-content $using:logFilePath -value "$(get-date -Format 'yyyy-MM-dd HH:mm:ss.ffff K') [Set-InstallDirector] Running the CVAD Installer"

                Start-Process -FilePath $using:xenDesktopServerSetupPath -ArgumentList "/components DESKTOPDIRECTOR /no_remote_assistance /noreboot /configure_firewall /passive" -Wait
            ...
            TestScript           = {
                try {
                    Add-content $using:logFilePath -value "$(get-date -Format 'yyyy-MM-dd HH:mm:ss.ffff K') [Test-InstallDirector] Checking if CVAD components have been installed"
        ...
        #region Director Configuration
        Script ConfigDirector {
            DependsOn            = '[Script]InstallDirector'
            ...
            TestScript           = {
                try {
                    Start-Process -FilePath $using:directorConfigPath -ArgumentList "/ddc $using:ddcDNSName"  -Wait 
                    Add-content $using:logFilePath -value "$(get-date -Format 'yyyy-MM-dd HH:mm:ss.ffff K') [Set-InstallDirector] Set DDC address for Director Successfully"
                    return $true
                }
                catch {
                    Add-content $using:logFilePath -value "$(get-date -Format 'yyyy-MM-dd HH:mm:ss.ffff K') [Set-InstallDirector] Fail to set DDC address for Director with $using:ddcDNSName"
                    return $true
                }
            }
        }
        ...
        Script DownloadCertificateRequestScript {
            DependsOn            = '[Script]ConfigDirector'
